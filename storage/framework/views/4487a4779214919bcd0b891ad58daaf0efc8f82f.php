  ...
  <table style="margin-top: 10px;width: 100%; border-collapse: collapse;" id="tbl_list_cart" cellpadding="5" bordercolor="#CCCCCC" border="1">
      ...
      <thead class="tbd-cart">
        ...
        <?php $i=0;$a=0;$b=0; $array_rowid=array("0");?>
     		 <?php $__currentLoopData = Cart::content(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?>
          <?php
          ...
          ?>
                ...
                echo $i; ?></td>
             		<td>
                    <img src="upload/product/<?php echo e($row->options->img); ?>" style="vertical-align: middle; margin-right: 10px; float:left; width:100px;">
                    <div style="margin-left:120px;">
                      <a href="<?php echo e(url('item/'.$row->options->namekd)); ?>" >
...
<td><input type="text" name="qty[]" value="<?php echo $row->qty; ?>" size="3" id="mySelect" onchange="myFunction()"></td>
                  ...
                  <input type="text" id="demo_price" style="display:none;" value="<?php echo e($row->price); ?>"></td>
                <?php $item_total=($row->qty)*($row->price); ?>
             		<td style="font-weight:bold">$<?php echo number_format($row->total); ?> VND</td>
                <td>
                  <a href="<?php echo e(url('del-item/'.$row->rowId)); ?>" onclick="return kiemtra();">
           ...
           <input type="text" name="rowid" value="<?php echo e(json_encode($array_rowid)); ?>" style="display:none">
  ...
  <div class="buy-cart" <?php
   if (Cart::total() == 0){
     ...
     ?>>
    <a href="<?php echo e(url('tin-tuc/mua-tra-gop')); ?>" class="btn-orange">Mua trả góp</a>
    <a href="<?php echo e(url('gio-hang-buoc-2')); ?>" class="btn-red">Tiếp tục</a>
    <button type="submit" style="border:none" class="btn-red update-cart" name="button">Cập nhật</button>

    <a href="<?php echo e(url('')); ?>" class="btn-blue">Mua thêm sản phẩm khác</a>
  </div>
